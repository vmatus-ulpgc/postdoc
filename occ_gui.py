...
def set_cam_parameters(): #parameters_array = [t_exp, fps, ag, dg, rg, bg]
    parameters_array = [0]*6
    for i in range(len(params_array)):
        parameters_array[i] = params_sliders[i].get()
    print("setting camera parameters")
    camera.resolution = (width,height) #set resolution (width,height) in pixels
    camera.shutter_speed = parameters_array[0] #set exposure time in microseconds
    camera.framerate = parameters_array[1]#set framerate in fps
    ...
    mmal.mmal_port_parameter_set_rational(camera._camera.control._port, 
                                          ...
                                          to_rational(parameters_array[2]))
    mmal.mmal_port_parameter_set_rational(camera._camera.control._port, 
                                          MMAL_PARAMETER_DIGITAL_GAIN ,
                                          to_rational(parameters_array[3]))
    camera.awb_mode = 'off'
    camera.awb_gains = (parameters_array[4],parameters_array[5])
...
shutter_button.grid(column=3, row=3)
...
for i in range(len(params_array)):
    ...
    #create entry:
    #if i<1:
    #    var = tkinter.IntVar(value=params_array[i])
    #else:
    var = tkinter.DoubleVar(value=params_array[i])
    params_vars.append(var)
    entry = tkinter.Entry(params_frame, width = 5, textvariable=params_vars[i])
...
set_params_button = tkinter.Button(master=params_frame, text="set", command=set_cam_parameters).grid(column=3, row=2)
